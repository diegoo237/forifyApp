...
///1 recenbendo os dados de receita
const showRecipe = async function() {
    try {
        const id = window.location.hash.slice(1);
        if (!id) return;
        renderSpiner(recipeContainer);
        const res = await fetch(`https://forkify-api.herokuapp.com/api/v2/recipes/${id}?key=167151e9-048a-4292-b9dc-200f3172875b`);
        ...
        `;
        recipeContainer.insertAdjacentHTML("beforeend", recipeContent);
    } catch (err) {
        alert(err);
    }
};
